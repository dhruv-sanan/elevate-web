    ...
    <section className="py-24 relative overflow-hidden bg-slate-900 dark:bg-black">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10" ref={ref}>
        <div className="text-center mb-16">
          ...
          >
            Real results for real businesses. We don't just build web apps; we build revenue engines.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 lg:gap-12">
          {stats.map((stat, index) => (
            <motion.div
              ...
              className="flex flex-col items-center text-center p-4 md:p-6 rounded-2xl md:rounded-3xl bg-white/5 border border-white/10 backdrop-blur-sm hover:bg-white/10 transition-colors"
            >
              <div className="text-3xl md:text-5xl lg:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-400 mb-1 md:mb-2 font-space-grotesk tracking-tight">
                {inView ? (
                  <CountUp
                    start={0}
                    end={stat.value}
              ...
              <h3 className="text-sm md:text-xl font-bold text-white mb-1 md:mb-2">{stat.label}</h3>
              <p className="text-xs md:text-sm text-slate-400">{stat.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
